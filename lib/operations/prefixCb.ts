...
const shiftLeftHlAddress = (mode: ShiftMode): OperationInfo => {
  return {
    operation: (cpu: GameboyCpu) => {
...
const shiftRightHlAddress = (mode: ShiftMode): OperationInfo => {
  return {
    operation: (cpu: GameboyCpu) => {
      const address = cpu.registers.getWord(Register16.HL);
      const byte = cpu.memory.getByte(address);
...
const swapRegister8 = (register8: Register8): OperationInfo => {
  return {
    operation: (cpu: GameboyCpu) => {
      const byte = cpu.registers.getByte(register8);
...
const swapHlAddress: OperationInfo = {
  operation: (cpu: GameboyCpu) => {
    const address = cpu.registers.getWord(Register16.HL);
    const byte = cpu.memory.getByte(address);
...
const testBitOfHlAddress = (bitIndex: number): OperationInfo => {
  return {
    operation: (cpu: GameboyCpu) => {
      const address = cpu.registers.getWord(Register16.HL);
      const byte = cpu.memory.getByte(address);
...
const setBitOfHlAddress = (bitIndex: number, value: boolean): OperationInfo => {
  return {
    operation: (cpu: GameboyCpu) => {
      const address = cpu.registers.getWord(Register16.HL);
      const byte = cpu.memory.getByte(address);

      const resultByte = setNthBit(byte, bitIndex, 0);
